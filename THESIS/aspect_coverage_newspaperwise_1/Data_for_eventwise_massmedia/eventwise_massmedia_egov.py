...
db = client['media-db']
egov_article_data = np.loadtxt("egov_article_data_20.csv",dtype=str,delimiter=";")
url_list = egov_article_data[:,9]
word_count = egov_article_data[:,2]
sent_pos = egov_article_data[:,7]
sent_neg = egov_article_data[:,8]
sent = egov_article_data[:,6]


coll = db['e-governance_articles_Final']
articles = coll.find({})

newspaper_list = ['thehindu','hindustantimes','timesofindia','telegraph','newindianexpress','indianexpress','deccanherald']
fp1=open('event_wise_count_category_massmedia_egov','w')
