...
db = client['media-db']
DI_article_data = np.loadtxt("DI_article_data_20.csv",dtype=str,delimiter=";")
url_list = DI_article_data[:,9]
word_count = DI_article_data[:,2]
sent_pos = DI_article_data[:,7]
sent_neg = DI_article_data[:,8]
sent = DI_article_data[:,6]

coll = db['DigitalIndia_articles_Final']
articles = coll.find({})

newspaper_list = ['thehindu','hindustantimes','timesofindia','telegraph','newindianexpress','indianexpress','deccanherald']
fp1=open('event_wise_count_category_massmedia_DI','w')
